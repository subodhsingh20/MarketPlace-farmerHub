...
function Navbar() {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-[100] border-b border-slate-200/90 bg-white/95 shadow-[0_20px_50px_rgba(15,23,42,0.14)] backdrop-blur-2xl backdrop-saturate-150">
        <div className="mx-auto max-w-7xl px-3 sm:px-5 lg:px-6">
          <div className="flex h-16 items-center justify-between gap-2 lg:h-[4.5rem] lg:gap-3 xl:gap-5">
            ...
            >
              <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-400 via-emerald-500 to-lime-500 shadow-[0_14px_30px_rgba(16,185,129,0.32)] ring-1 ring-white/20 transition-all duration-300 group-hover:scale-105 lg:h-11 lg:w-11">
                <span className="text-base font-bold text-slate-950 lg:text-lg">FM</span>
              </div>
              <div className="hidden min-w-0 sm:block">
                <h1 className="max-w-[200px] text-[0.98rem] font-bold leading-tight text-slate-950 transition-colors duration-300 group-hover:text-emerald-700 lg:max-w-[250px] lg:text-[1.06rem] xl:max-w-[320px]">
                  Farmer Marketplace 
                </h1>
                <p className="mt-0.5 text-[0.78rem] text-slate-600 transition-colors duration-300 group-hover:text-emerald-700 lg:text-[0.84rem]">
            ...
            <div className="hidden flex-1 items-center justify-center gap-0.5 px-2 lg:flex xl:gap-1 xl:px-4">
              {links.map((link) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `relative rounded-2xl px-3 py-2 text-[0.94rem] font-semibold transition-all duration-300 hover:scale-[1.01] xl:px-3.5 ${
                      isActive
                        ? "bg-gradient-to-r from-emerald-400 to-emerald-500 text-slate-950 shadow-[0_12px_24px_rgba(16,185,129,0.32)]"
            ...
            <div className="flex flex-shrink-0 items-center gap-2 lg:gap-2.5">
              {/* User Info */}
              {user && (
                <div className="hidden items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 shadow-sm lg:flex">
                  <span className="rounded-full bg-gradient-to-r from-emerald-300 to-lime-300 px-3 py-1 text-[0.84rem] font-semibold capitalize text-slate-950">
                    {user.role}
                  </span>
                  <span className="hidden max-w-[110px] truncate text-[0.92rem] font-semibold text-slate-900 xl:inline">
                    {user.name}
                  </span>
                </div>
              )}
                ...
                <div className="relative" ref={cartShellRef}>
                  <button
                    ...
                    className={`relative flex items-center gap-2 rounded-xl px-3 py-2 font-semibold transition-all duration-300 hover:scale-[1.01] xl:px-3.5 ${
                      isCartOpen
                        ? "bg-gradient-to-r from-emerald-400 to-emerald-500 text-slate-950 shadow-[0_12px_24px_rgba(16,185,129,0.26)]"
              ...
              {user && (
                <button
                  ...
                  className="hidden items-center gap-2 rounded-xl bg-rose-50 px-3 py-2 font-semibold text-rose-700 transition-all duration-300 hover:scale-[1.01] hover:bg-rose-100 hover:text-rose-900 md:flex xl:px-3.5"
                >
              ...
              <button
                type="button"
                className="flex flex-col space-y-1 rounded-xl p-2 transition-colors duration-300 hover:bg-slate-100 lg:hidden"
                onClick={() => setIsMenuOpen(!isMenuOpen)}
                aria-label="Toggle menu"
              >
                <span className={`w-6 h-0.5 bg-slate-900 transition-all duration-300 ${isMenuOpen ? 'rotate-45 translate-y-1.5' : ''}`}></span>
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `block rounded-xl px-4 py-3 text-[0.98rem] font-semibold transition-all duration-300 ${
                      isActive
                        ? "bg-gradient-to-r from-emerald-400 to-emerald-500 text-slate-950 shadow-lg"
                        : "text-slate-700 hover:text-slate-950 hover:bg-slate-100"
                    }`
                  }
                >
              ...
              {user && (
                <div className="space-y-3 px-4 py-2">
                  <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3">
                    ...
                    <span className="min-w-0 truncate text-[0.94rem] font-medium text-slate-900">{user.name}</span>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="flex w-full items-center justify-center gap-2 rounded-xl bg-rose-50 px-4 py-3 font-semibold text-rose-700 transition-all duration-300 hover:bg-rose-100 hover:text-rose-900"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                    </svg>
